...
const EditPin = ({ postSearchSelectors }) => {
  const { pinId } = useParams();
  const { post } = postSearchSelectors;
  const [loading, setLoading] = useState(false);
  const [fields, setFields] = useState(false);
  const [imageAsset, setImageAsset] = useState(post?.selectedFile);
  const [wrongImageType, setWrongImageType] = useState(null);
  const [title, setTitle] = useState(post?.title);
  const [about, setAbout] = useState(post?.message);
              ...
              >
                ...
                {categories.map((categoryOption) => (
                  <option
                    ...
                    selected={category === categoryOption.name ? true : false}
                  >
